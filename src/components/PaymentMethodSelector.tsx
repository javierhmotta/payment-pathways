  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Choose how you'd like to pay your service fee invoices
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        {selectedMethodData && (
          <div className={cn(
            ...
            <Check className={cn(
              "w-5 h-5",
              selectedMethodData.discount ? "text-amber-600" : "text-emerald-600"
            )} />
            <div className="text-sm">
              <span className={cn(
                "font-medium",
                selectedMethodData.discount ? "text-amber-800 dark:text-amber-300" : "text-emerald-800 dark:text-emerald-300"
              )}>
                {selectedMethodData.name} selected
              </span>
              {selectedMethodData.discount && (
                <span className="text-amber-700 dark:text-amber-400"> — You'll receive a {selectedMethodData.discount}% discount!</span>
              )}
              {selectedMethodData.type === "recurring" && (
              ...
              {selectedMethodData.type === "manual" && (
                <p className={cn(
                  "mt-1",
                  selectedMethodData.discount ? "text-amber-700 dark:text-amber-400" : "text-emerald-700 dark:text-emerald-400"
                )}>
...
function PaymentOptionCard({ method, isSelected }: { method: PaymentMethod; isSelected: boolean }) {
  return (
    <Label
      htmlFor={method.id}
      className={cn(
